...
| Assigned quiz list | `/student/quizzes` | Same data via new `GET /api/student/quiz/assigned` | = | Availability (not started / expired) is a client display hint using the same rules as the template; the backend still enforces `QUIZ_NOT_STARTED` / `QUIZ_EXPIRED` / `MAX_ATTEMPTS_REACHED`. |
