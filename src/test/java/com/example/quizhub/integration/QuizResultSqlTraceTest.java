...
public class QuizResultSqlTraceTest {
    ...
    private Map<String, Integer> runTrace(int questionCount) {
        // --- 1. CLEANUP OLD DATA ---
        ...
        Long attemptId = transactionTemplate.execute(status -> {
            Quiz quiz = quizRepository.save(Quiz.builder()
                    .title("Trace Scale Quiz " + questionCount)
        ...
        // Normalize and Group SQLs
        Map<String, Integer> templateCounts = new LinkedHashMap<>();
        for (String sql : sqls) {
            String normalized = normalizeSql(sql);
            templateCounts.put(normalized, templateCounts.getOrDefault(normalized, 0) + 1);
        }
    ...
    private String normalizeSql(String sql) {
        ...
        // simple normalization to remove exact values in IN clauses
        trimmed = trimmed.replaceAll("in \\([^)]+\\)", "in (...)");
